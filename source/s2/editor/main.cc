...
// namespace s2 {
// namespace {
// template <typename E> using target_t = E::target_type;
// template <typename T> struct simple_codec {
//   using target_type = T;
//   auto encode(auto& backend, target_type const& v) { return
//   backend.encode(v); } auto decode(auto& backend, auto&& input) ->
//   std::optional<target_type> {
//     return backend.template decode<target_type>(input);
//   }
// };
// template <typename C> struct field_codec {
//   using target_type = target_t<C>;
//   C c;
//   std::string_view name;
//   std::optional<target_type> default_value;
//   auto encode(auto& backend, target_type const& v) {
//     return c.encode(backend, v);
//   }
//   auto decode(auto& backend, auto&& input) -> std::optional<target_type> {
//     auto x = input.get_field(name);
//     auto y = c.decode(backend, x);
//     return y;
//   }
// };
// template <typename T, typename... Fields> struct struct_codec {
//   using target_type = T;
//   std::tuple<Fields...> fields;
//   auto encode(auto& backend, target_type const& t);
//   auto decode(auto& backend, auto& input) {
//     std::tuple results =
//       fields.map([&](auto f) { return f.c.decode(backend, input); });
//     return T{results};
//   }
// };
// template <typename T, typename... Fs>
// constexpr auto make_struct(Fs&&... fs) -> struct_codec<T, Fs...> {
//   return {std::make_tuple(std::forward<Fs>(fs)...)};
// }
// template <typename C, typename F> struct guard_codec {
//   using target_type = target_t<C>;
//   C c;
//   F f;
//   auto encode(auto& backend, target_type const& v) {
...
//   }
//   auto decode(auto& backend, auto& input) {
//     auto x = c.decode(backend, input);
//     if (x.has_value() && f(x.value()))
//       return x;
//     return std::nullopt;
//   }
// };
// template <typename C> struct vector_codec {
//   using target_type = std::vector<C>;
//   C c;
//   auto encode(auto& backend, target_type const& v) {}
//   auto decode(auto& backend, auto& input) {}
// };
...
// // clang-format off
// [[maybe_unused]]
// constexpr auto foo_codec =
//   make_struct<foo>(
//     field_codec{simple_codec<int>{}, "bar", std::nullopt},
//     field_codec{
//       guard_codec{
//         simple_codec<bool>{},
...
//       "baz", std::nullopt},
//     field_codec{
//       vector_codec{simple_codec<bool>{}},
//       "xxx",
//       std::nullopt,
//     }
//   );
